...
app.get('/facts', function(req, res){
    if('topic' in req.query && 'num' in req.query){
        ...
        var valid_topics = ['red', 'blue', 'green', 'yellow'];
        if(valid_topics.includes(topic)){
            ...
            if(topic == 'yellow'){
                var facts = ["Just-ripe bananas and banana slugs are yellow", "Cadmium can give a very beautiful yellow, but use at your own risk", "Yellow flames appear when sodium is a contaminant", "Yellow journalism is typically not very reputable", "Class F stars appear yellow"]
            }
            if(num > 0 && num <= 5){
